...
namespace Bebidas
{
  class TDevolucionSBO : TvkBaseForm, IvkFormInterface
  {
    private int nError;
    private Form oForm;
        ...
        if ((!BusinessObjectInfo.BeforeAction) && (BusinessObjectInfo.EventType == BoEventTypes.et_FORM_DATA_ADD) && (BusinessObjectInfo.ActionSuccess))
        {
          ...
          if (!String.IsNullOrEmpty(DocEntry))
          {
            ...
            String StrSql = !GlobalSettings.RunningUnderSQLServer ?
                ...
                String.Format("SELECT TOP 1 BaseEntry FROM RDN1 T0 Where T0.DocEntry = {0} AND T0.BaseType =  '15' AND (T0.U_VID_Bonif1 = 'Y' OR  T0.U_VID_Bonif2 = 'Y')", DocEntry);
            ...
            oRecordSet.DoQuery(String.Format("SELECT DocNum, DocDate FROM ORDN Where DocEntry = {0}", DocEntry));
            ...
            StrSql = !GlobalSettings.RunningUnderSQLServer ?
                String.Format("", DocEntry) :
                String.Format("SELECT (SELECT COUNT(*) FROM RDN1 T0 Where T0.DocEntry = {0} AND T0.BaseType =  '15' AND (T0.U_VID_Bonif1 = 'Y' OR  T0.U_VID_Bonif2 = 'Y')) Lineas  FROM ORDN Where DocEntry = {0}", DocEntry);
            oRecordSet.DoQuery(StrSql);
            if ((Int32)oRecordSet.Fields.Item("Lineas").Value > 0)
            {
              ...
              StrSql = !GlobalSettings.RunningUnderSQLServer ?
                  String.Format("", DocEntry) :
                  String.Format("SELECT ItemCode, U_VID_Bonif1, U_VID_Bonif2, U_VID_QtyBon1, U_VID_QtyBon2, U_VID_ArtBon2, U_VID_CtaBon, WhsCode, OcrCode, OcrCode2, OcrCode3, OcrCode4, OcrCode5, U_VID_LoteB1, U_VID_LoteB2 FROM RDN1 T0 Where DocEntry = {0} AND  (T0.U_VID_Bonif1 = 'Y' OR  T0.U_VID_Bonif2 = 'Y')", DocEntry);
              oRecordSet.DoQuery(StrSql);
              ...
              if (LRes == 0)
              {
                ...
                StrSql = !GlobalSettings.RunningUnderSQLServer ?
                    ...
                    String.Format("UPDATE ORDN SET U_VID_NumDev = {1} Where DocEntry = {0}", DocEntry, DocEntrySalida);
                oRecordSet.DoQuery(StrSql);
              }
              else
              {
